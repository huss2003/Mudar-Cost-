  ...
  try {
    ...
    // PUT /drawings/:id — replace file (e.g. convert PDF→PNG then re-upload)
    {
      ...
      if (m && method === 'PUT') {
        ...
        await adminClient.from('drawings').update({ status: 'uploaded' }).eq('id', drawingId);
        // Use the same detection pipeline as the main detect endpoint
        await detectDrawing(drawingId).catch((e) => console.error('[auto-compute-put]', e));
        return ok({
